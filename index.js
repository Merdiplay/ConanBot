...
bot.on("ready", () => {
    bot.user.setGame('Fälle lösen!'); //you can set a default game
...
bot.on("message", async message => { 
    ...
    if (message.content.indexOf(config.prefix) === 0) { // Message starts with your prefix
        ...
        if (cmd === 'hi' || cmd === 'hallo') { // the first command [I don't like ping > pong]
            message.channel.send(`Hallo ${message.author.toString()}`);
            return; 
        }
